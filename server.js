...
// Same-Origin Still Snapshot Proxy — used by the camera grid instead of the
// permanent /api/stream MJPEG connection above. Browsers cap concurrent
// connections to one origin at ~6, and every grid tile holding its own
// indefinite MJPEG stream open exhausts that past 5 cameras — any camera
// beyond that queues forever and never renders (confirmed against a real
// 7-camera Frigate server). A short-lived snapshot request per poll avoids
// ever holding more than a handful of connections open at once, regardless
// of how many cameras are in the grid.
app.get('/api/snapshot/:cameraName', (req, res) => {
  const { cameraName } = req.params;
  const frigateIP = settings.go2rtcHost || '192.168.2.210';
  const url = `http://${frigateIP}:5000/api/${cameraName}/latest.jpg`;
  ...
  proxyReq.on('error', (err) => {
    console.error(`[SNAPSHOT PROXY ERROR] Failed to proxy snapshot for "${cameraName}":`, err.message);
    res.status(500).send('Snapshot proxy failed');
  });
