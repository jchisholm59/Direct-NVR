...
// Grid tiles use a polled still snapshot instead of a permanent MJPEG
// connection — see GRID_SNAPSHOT_POLL_MS below for why. The full-screen
// zoom view still uses getMjpegUrl() for a true continuous stream, since
// only one of those is ever open at a time.
function getSnapshotUrl(camName) {
  return resolveUrl(`/api/snapshot/${camName}`);
}
...
function restartStreams() {
  ...
  Object.keys(appCameras).forEach(camName => {
    ...
    if (img) {
      if (img.tagName === 'IMG') {
        // Grid tiles are snapshot-polled, not a persistent MJPEG connection
        // — see getSnapshotUrl().
        img.src = `${getSnapshotUrl(camName)}?t=${Date.now()}`;
...
function renderCameraGrid() {
  ...
  cameraNames.forEach(camName => {
    const cam = appCameras[camName];
    const snapshotUrl = getSnapshotUrl(camName);
    ...
    card.innerHTML = `
      ...
      <div class="video-container relative overflow-hidden bg-black cursor-pointer group" id="container-${camName}" onclick="zoomCamera('${camName}')">
        ${appSettings.connectionMode === 'webrtc' 
          ...
          : `<img id="video-${camName}" class="video-iframe object-cover select-none pointer-events-none" src="${snapshotUrl}" />`
        }
...
// Helper to reconstruct and refresh a camera <img> element cleanly. The
// zoom player (one at a time) uses the true continuous MJPEG stream; every
// other <img> is a grid tile, which uses the polled snapshot instead — see
// getSnapshotUrl() for why.
function refreshImageElement(img, camName) {
  if (!img) return;
  if (img.id === 'zoom-player') {
    const go2rtcIP = getGo2rtcIP();
    const port = appSettings.go2rtcPort || '5000';
    const cam = appCameras[camName];
    if (cam) {
...
// Poll interval for grid tile snapshots. Browsers cap concurrent connections
// per origin at ~6; the old approach (every grid tile holding its own
// indefinite MJPEG connection through /api/stream) silently starved any
// camera past the 5th — confirmed against a real 7-camera Frigate server,
// where cameras 6 and 7 queued forever and never rendered. A short-lived
// snapshot request per tile per tick keeps the grid under that cap
// regardless of camera count.
const GRID_SNAPSHOT_POLL_MS = 2000;
setInterval(() => {
  if (appSettings.connectionMode !== 'mjpeg') return; // webrtc tiles are <video>, not polled
  Object.keys(appCameras).forEach(camName => {
    const img = document.getElementById(`video-${camName}`);
